...
public abstract class PartieOie extends Partie {
	...
	public void initialisation(Scanner sc) {
		...
		do {
			...
			this.addParticipants(new JoueurOie(sc.nextLine()));
	...
	/**
	public int occupeCase(int pos, Joueur j) {
		Cases tmp = this.getP().getCases(pos);
		this.libereCase(j.getPosition());
		if (pos >= p.getTaille()) {
			pos = finDuJeu(pos, this.getDe1().getDe() + this.getDe2().getDe());
		} else {
			while (tmp.estOccupee()) {
				tmp = this.getP().getCases(pos);
				if (tmp instanceof CasesLiberation) {
					((CasesLiberation) tmp).gestionLiberation(pos, j, this);
				}
				pos--;
			}
		}
	...
	// Jouer un tour = lancer dès + déplacement public int jouer(Joueur
	joueur)

	{
		int posCour = joueur.getPosition();
		this.libereCase(posCour);
		int newPos;
		newPos = joueur.joueTour(this.getDe1(), this.getDe2());
		if (newPos >= p.getTaille()) {
			newPos = this.finDuJeu(posCour, de1.getDe() + de2.getDe());
		}
		newPos = this.occupeCase(newPos, joueur);
...
**/
	...
	public int jouer(JoueurOie joueur){
		...
		int newPos=joueur.joueTour(this.getDe1(), this.getDe2());
		Cases tmp ;
